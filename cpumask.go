...
func (c *Cpumask) ToHexString() string {
	if len(c.mask) == 0 {
		return "0"
	}

	hexParts := make([]string, len(c.mask))
	for i, block := range c.mask {
		hexParts[len(c.mask)-1-i] = fmt.Sprintf("%016x", block)
	}

	hexString := strings.TrimLeft(strings.Join(hexParts, ""), "0")
	if hexString == "" {
		return "0"
	}
	return hexString
...
func CpumaskFromStr(s string) (*Cpumask, error) {
